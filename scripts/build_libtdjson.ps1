...
#
# 缓存:与 build_libtdjson.sh 同一套机制——产物目录放一个 `.tdlib-version`
# manifest(TDLib 版本 + 平台)。若产物存在且 manifest 匹配锁定版本,直接跳过
# 编译进 ctypes 验证;CI 里配合 actions/cache 持久化产物目录,避免每次发布
# 重编 TDLib。注意:**不缓存 vcpkg binary cache**——runner 预装的 vcpkg 每天
# 更新,abi 哈希随之变化,二进制缓存 2~3 天后必然 miss 又要冷编译(~2h);
# 产物目录 + manifest 只依赖本项目自己声明的版本,与 vcpkg 版本无关,稳定命中。
#
...
# 与 build_libtdjson.sh 锁定同一 TDLib 版本(vcpkg port 当前即此版本)。
# 需要强制重编时改这里即可(manifest 不匹配自然 miss)。
$TDLIB_VERSION = "1.8.46"

# ---- 0. 产物缓存判断:版本/平台未变则跳过编译 ----
# 与 build_libtdjson.sh 的 `.tdlib-version` manifest 同格式,CI 缓存产物目录。
$MANIFEST = Join-Path $DestDir ".tdlib-version"
$CURRENT_MANIFEST = "tdlib=$TDLIB_VERSION windows/x64"

if (-not (Test-Path $Dest) -or -not (Test-Path $MANIFEST) -or
    ((Get-Content $MANIFEST -Raw).Trim() -ne $CURRENT_MANIFEST)) {
    ...
    # 不再重编 tdlib(本机 / 同一 runner 会话内有效;跨 run 持久化靠
    # actions/cache 缓存产物目录,见文件头注释)。
    ...
    # 写 manifest,后续 CI / 本地命中即跳过编译
    $CURRENT_MANIFEST | Out-File -FilePath $MANIFEST -Encoding ascii
    Get-ChildItem $DestDir | Select-Object Name, Length | Format-Table -AutoSize
} else {
    Write-Host "==> 命中缓存产物 $Dest (TDLib $TDLIB_VERSION 未变),跳过编译"
}

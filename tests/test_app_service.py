...
async def test_app_login_state_machine(bus) -> None:
    ...
    app = await _make_app(bus, client_state="phone_required")
    assert await app.get_login_state() == "phone_required"
    # FakeTelegramClient.state 是 property(无 setter);通过 _state 改底层
    app.client._state = "ready"  # type: ignore[attr-defined]
    assert await app.get_login_state() == "ready"
...
async def test_app_login_without_credentials_raises(bus) -> None:
    ...
    app = await _make_unconfigured_app(bus)
...
async def test_app_subscribe_unsubscribe(app: AppService, collected: list) -> None:
    ...
    await app.subscribe_channel(chan)
    # subscribe 路径:upsert_channel + set_channel_subscribed(True) 均被调
    app._sub._storage.upsert_channel.assert_awaited_once_with(chan)  # type: ignore[attr-defined]
    app._sub._storage.set_channel_subscribed.assert_awaited_with(42, True)  # type: ignore[attr-defined]
    await app.unsubscribe_channel(42)
    # unsubscribe 路径:set_channel_subscribed(42, False)
    app._sub._storage.set_channel_subscribed.assert_awaited_with(42, False)  # type: ignore[attr-defined]
...
async def test_unsubscribe_storage_failure_raises_no_false_success(
    ...
    """
    app._sub._storage.set_channel_subscribed = AsyncMock(  # type: ignore[attr-defined]
...
async def test_unsubscribe_failure_does_not_publish_event(app: AppService, bus) -> None:
...
async def test_list_messages_falls_back_to_storage_truth(app: AppService) -> None:
    ...
    app._sub.list_messages = AsyncMock(return_value=expected)  # type: ignore[attr-defined]
    out = await app.list_messages()
    assert out is expected
    app._sub.list_messages.assert_awaited_once()  # type: ignore[attr-defined]
    # channel_ids 应是 None(没被 facade 补 [])
    call_args = app._sub.list_messages.await_args  # type: ignore[attr-defined]
    assert call_args.args[0] is None or call_args.kwargs.get("channel_ids") is None


async def test_no_subscribed_attribute_remains(app: AppService) -> None:
    ...
    val = getattr(app, "_subscribed", "__missing__")
...
async def test_app_list_messages_include_unsubscribed_returns_all_channels(
    ...
    app._sub.list_messages = AsyncMock(return_value=expected)  # type: ignore[attr-defined]
    out = await app.list_messages(include_unsubscribed=True)
    assert out is expected
    kwargs = app._sub.list_messages.await_args.kwargs  # type: ignore[attr-defined]
...
async def test_app_list_messages_channel_ids_explicit_overrides_include_unsubscribed(
    ...
    """
    app._sub.list_messages = AsyncMock(return_value=[])  # type: ignore[attr-defined]
    await app.list_messages(channel_ids=[10, 20], include_unsubscribed=True)
    call_args = app._sub.list_messages.await_args  # type: ignore[attr-defined]
...
async def test_search_messages_scope_all_passes_include_unsubscribed(
    ...
    """
    app._sub.list_messages = AsyncMock(return_value=[])  # type: ignore[attr-defined]
    await app.list_messages(search="tech", include_unsubscribed=True)
    kwargs = app._sub.list_messages.await_args.kwargs  # type: ignore[attr-defined]

...
# libtdjson 的 ctypes 绑定 — 改编自 aiotdlib (MIT) 的 tdjson.py:
# - ujson → 标准库 json(零额外依赖);
# - 其余保留:动态库定位 / 函数签名 / 异步桥接逻辑。
import asyncio
import json
import logging
import pathlib
...
TDJsonQuery = str | bytes | dict
LogMessageCallback = CFUNCTYPE(None, c_int, c_char_p)
LogMessageCallbackT = Callable[[int, str], None]
...
def _get_bundled_tdjson_lib_path() -> str:
    """定位 libtdjson 动态库:优先系统库,其次包内 `tdlib/` 目录。"""
...
class CoreTDJson:
    ...
    def __init__(self, library_path: str | pathlib.Path):
        ...
        self._td_receive: Callable[[float], bytes] = self._tdjson.td_receive
        ...
        # td_set_log_message_callback(int max_verbosity_level, callback)
        # 注意:TDLib 1.8+ 是双参数签名(早期 aiotdlib 适配的旧版只有 callback 一个
        # 参数),只传一个参数会让 C 侧把函数指针读成 max_verbosity_level、把寄存器
        # 垃圾值当回调指针,后续 TDLib 线程回调垃圾地址 → SIGBUS。
        self._td_set_log_message_callback: Callable[[int, LogMessageCallbackT], None] = (
            self._tdjson.td_set_log_message_callback
        )
        self._td_set_log_message_callback.restype = None
        self._td_set_log_message_callback.argtypes = [c_int, LogMessageCallback]
        # 必须持有回调的 Python 侧引用:TDLib 内部线程后续触发日志回调时,
        # 若临时对象已被 GC 回收会访问已释放内存(经典 ctypes 陷阱,SIGBUS)。
        self._log_message_callback = LogMessageCallback(self.__log_message_callback)
        self._td_set_log_message_callback(TDLibLogVerbosity.MAXIMUM, self._log_message_callback)
    ...
    def execute(self, query: TDJsonQuery) -> dict | None:
        query = _encode_tdjson_query(query)
    ...
    def close_client(self, client_id: int):
        # TDLib client 实例在 close 后自动销毁
        self.send(client_id, {"@type": "close"})


class TDJson(CoreTDJson):
    """在 CoreTDJson 之上加订阅分发:多个 client 共享一个接收循环。"""
...
class TDJsonClient:
    """异步桥接:把 C 层的事件循环收到的新事件通过队列送给 asyncio 侧。"""

    @classmethod
    def create(cls, library_path: str | None = None):
    ...
    def __init__(self, td_json: TDJson):
        ...
        self.client_id: int | None = td_json.create_client_id()
    ...
    async def execute(self, query: TDJsonQuery) -> dict | None:
    ...
    async def receive(self) -> AsyncGenerator[dict]:
        while True:
            try:
                message: Any = await self._updates_queue.get()
                yield message
            except (asyncio.CancelledError, KeyboardInterrupt):
                self._cleanup()
                raise
